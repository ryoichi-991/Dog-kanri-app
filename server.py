...
@media(max-width:700px){{.owner-view main{{padding:8px 0}}.owner-view main>.card{{padding:18px 14px;border-radius:0;border-left:0;border-right:0}}h1{{font-size:23px;line-height:1.35}}h2{{font-size:18px}}input,select,textarea{{font-size:16px;min-height:46px}}button,.button{{min-height:44px;padding:11px 14px}}.health-desktop-only,.calendar-desktop-only{{display:none!important}}.health-mobile-only,.calendar-mobile-only{{display:block}}.health-toolbar{{display:grid;grid-template-columns:1fr 1fr;gap:7px}}.health-toolbar .button{{display:flex;align-items:center;justify-content:center;text-align:center;font-size:12px;min-height:48px;padding:8px}}.health-entry-form{{margin-left:-5px;margin-right:-5px;padding:15px 12px}}.health-entry-form>.grid{{grid-template-columns:1fr;gap:4px}}.health-entry-form>button{{position:sticky;bottom:8px;z-index:4;width:100%;min-height:52px;font-size:16px;box-shadow:0 7px 22px #55333f44}}.health-month-nav{{display:grid!important;grid-template-columns:1fr 1fr;gap:8px}}.health-month-nav h2{{grid-column:1/-1;grid-row:1;margin:0;text-align:center}}.health-month-nav .button{{margin-top:0;text-align:center}}}}
...
def calendar_page(month: str = "", calendar_category: str = "", calendar_state: str = "", access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    try:
    ...
    events = [item for item in events if first_day <= item[0] <= month_end and (not calendar_category or item[2] == calendar_category) and (not calendar_state or item[3] == calendar_state)]
