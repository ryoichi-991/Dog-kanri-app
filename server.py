...
def health_page(access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    dog_search_script = '''<style>.dog-picker{min-width:270px}.dog-search-all{display:flex;align-items:center;gap:7px;margin:8px 0 3px;font-weight:500}.dog-search-all input{width:auto;margin:0}.dog-search-count{display:block;margin-top:5px;color:#806b72}.dog-picker select{margin-top:2px}</style>
...
def family_dog_health(dog_id: int, user: User = Depends(require_user), session: Session = Depends(db)):
    ...
    if shared_ids.get("disease"):
        for item in session.scalars(select(DiseaseHistory).where(
            ...
            entries.append((item.diagnosed_on or date.min, "病歴", item.disease_name, item.details or ""))
