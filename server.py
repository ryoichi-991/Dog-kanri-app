...
FINANCE_CATEGORIES = {
    "sale": "仔犬販売", "deposit": "予約金・前受金", "stud": "交配料", "grooming": "トリミング",
...
def finance_period_close(session: Session, tenant_id: int, target_day: date) -> FinancePeriodClose | None:
    return session.scalar(select(FinancePeriodClose).where(
        ...
        FinancePeriodClose.year == target_day.year,
        FinancePeriodClose.month == target_day.month,
    ))
...
def finance_account_balance_on(session: Session, tenant_id: int, account: FinanceAccount, target_day: date) -> int:
    balance = account.opening_balance
    assignments = session.scalars(select(FinanceAccountEntry).where(FinanceAccountEntry.tenant_id == tenant_id, FinanceAccountEntry.account_id == account.id)).all()
    entry_ids = [item.financial_entry_id for item in assignments]
    if entry_ids:
        entries = session.scalars(select(FinancialEntry).where(FinancialEntry.tenant_id == tenant_id, FinancialEntry.id.in_(entry_ids), FinancialEntry.occurred_on <= target_day)).all()
        balance += sum(item.amount if item.entry_type == "income" else -item.amount for item in entries)
    transfers = session.scalars(select(FinanceAccountTransfer).where(FinanceAccountTransfer.tenant_id == tenant_id, FinanceAccountTransfer.transferred_on <= target_day, ((FinanceAccountTransfer.from_account_id == account.id) | (FinanceAccountTransfer.to_account_id == account.id)))).all()
    for transfer in transfers:
        balance += transfer.amount if transfer.to_account_id == account.id else -transfer.amount
    return balance
...
def finance_trial_balance_period(start_year: str, through_month: str, start_month: int) -> tuple[int, date, date]:
    try:
        selected_year = int(start_year) if start_year else (date.today().year if date.today().month >= start_month else date.today().year - 1)
        period_start, fiscal_end = finance_fiscal_period(selected_year, start_month)
        default_month = date.today() if period_start <= date.today() <= fiscal_end else fiscal_end
        selected_month = date.fromisoformat(f"{through_month}-01") if through_month else default_month.replace(day=1)
    except ValueError:
        raise HTTPException(status_code=400, detail="試算表の表示期間を確認してください")
    period_end = (selected_month.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(days=1)
    if selected_year < 2000 or selected_year > 2099 or selected_month < period_start or period_end > fiscal_end:
        raise HTTPException(status_code=400, detail="集計月は選択した事業年度の範囲内で指定してください")
    return selected_year, period_start, period_end
...
def finance_trial_balance_page(start_year: str = "", through_month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    user, tenant = access
...
def finance_trial_balance_csv(start_year: str = "", through_month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    _, tenant = access
    setting = session.scalar(select(FinanceFiscalSetting).where(FinanceFiscalSetting.tenant_id == tenant.id))
    start_month = setting.start_month if setting else 1
    selected_year, period_start, period_end = finance_trial_balance_period(start_year, through_month, start_month)
...
def finance_statement_report_data(session: Session, tenant_id: int, selected_year: int, period_start: date, period_end: date):
    accounts, subaccounts, journals, lines = finance_double_entry_data(session, tenant_id, period_start, period_end)
    signed_balances = {item.id: 0 for item in accounts}
    for line in lines:
        signed_balances[line.account_id] = signed_balances.get(line.account_id, 0) + (line.amount if line.side == "debit" else -line.amount)
    statement_balances: dict[int, int] = {}
    for account in accounts:
        signed = signed_balances.get(account.id, 0)
        statement_balances[account.id] = signed if account.account_type in {"asset", "expense"} else -signed
    return accounts, subaccounts, journals, lines, statement_balances
...
def finance_statements_report_page(start_year: str = "", through_month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    start_month = setting.start_month if setting else 1
    selected_year, period_start, period_end = finance_trial_balance_period(start_year, through_month, start_month)
    accounts, _, journals, lines, balances = finance_statement_report_data(session, tenant.id, selected_year, period_start, period_end)
    income_total = sum(balances[item.id] for item in accounts if item.account_type == "revenue"); expense_total = sum(balances[item.id] for item in accounts if item.account_type == "expense"); profit = income_total - expense_total
    asset_total = sum(balances[item.id] for item in accounts if item.account_type == "asset"); liability_total = sum(balances[item.id] for item in accounts if item.account_type == "liability"); equity_total = sum(balances[item.id] for item in accounts if item.account_type == "equity"); equation_difference = asset_total - liability_total - equity_total - profit
    profit_rows = "".join(f'<tr><td>{FINANCE_ACCOUNT_TYPES[item.account_type]}</td><td>{html.escape(item.code)} {html.escape(item.name)}</td><td>{"-" if balances[item.id] < 0 else ""}¥{abs(balances[item.id]):,}</td></tr>' for item in accounts if item.account_type in {"revenue", "expense"} and balances[item.id])
    balance_rows = "".join(f'<tr><td>{FINANCE_ACCOUNT_TYPES[item.account_type]}</td><td>{html.escape(item.code)} {html.escape(item.name)}</td><td>{"-" if balances[item.id] < 0 else ""}¥{abs(balances[item.id]):,}</td></tr>' for item in accounts if item.account_type in {"asset", "liability", "equity"} and balances[item.id])
    fiscal_month_options = "".join(f'<option value="{year:04d}-{month:02d}" {"selected" if period_end.year == year and period_end.month == month else ""}>{year}年{month}月</option>' for year, month in finance_fiscal_months(period_start))
    query_string = urlencode({"start_year": selected_year, "through_month": f"{period_end:%Y-%m}"})
    body = f'''<h1>損益計算書・貸借対照表</h1><p>複式仕訳の勘定科目残高から、事業年度の損益と指定月末の財政状態を確認します。</p>
    ...
    <p class="tenant">対象期間：{period_start}～{period_end}。期首残高・年度繰越・取消仕訳を含む複式仕訳 {len(journals)}伝票／{len(lines)}明細を集計しています。正式な決算書・税務申告は税理士と確認してください。</p><div class="health-toolbar"><a class="button secondary" href="/modules/finance/general-ledger?start_year={selected_year}">総勘定元帳・複式試算表</a></div>
    ...
    <h2>貸借対照表（{period_end}現在）</h2><div class="calendar-desktop-only" style="overflow-x:auto"><table><tr><th>区分</th><th>勘定科目</th><th>金額</th></tr>{balance_rows or '<tr><td colspan="3">対象期間の残高はありません。</td></tr>'}<tr><th colspan="2">資産合計</th><th>¥{asset_total:,}</th></tr><tr><th colspan="2">負債合計</th><th>¥{liability_total:,}</th></tr><tr><th colspan="2">純資産合計</th><th>¥{equity_total:,}</th></tr><tr><th colspan="2">当期利益</th><th>{'-' if profit < 0 else ''}¥{abs(profit):,}</th></tr><tr><th colspan="2">負債・純資産・当期利益合計</th><th>¥{liability_total + equity_total + profit:,}</th></tr><tr><th colspan="2">貸借差額</th><th class="{'error' if equation_difference else ''}">¥{abs(equation_difference):,}</th></tr></table></div><section class="calendar-mobile-only"><article class="calendar-mobile-card"><h3>資産合計 ¥{asset_total:,}</h3><p>負債 ¥{liability_total:,}／純資産 ¥{equity_total:,}／当期利益 {'-' if profit < 0 else ''}¥{abs(profit):,}</p><p>貸借差額 ¥{abs(equation_difference):,}</p></article></section>'''
    return layout("損益計算書・貸借対照表", body, user)
...
def finance_statements_report_csv(start_year: str = "", through_month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    accounts, _, _, _, balances = finance_statement_report_data(session, tenant.id, selected_year, period_start, period_end)
    rows = [["損益計算書" if item.account_type in {"revenue", "expense"} else "貸借対照表", FINANCE_ACCOUNT_TYPES[item.account_type], item.code, item.name, balances[item.id]] for item in accounts if balances[item.id]]
    income_total = sum(balances[item.id] for item in accounts if item.account_type == "revenue"); expense_total = sum(balances[item.id] for item in accounts if item.account_type == "expense")
    asset_total = sum(balances[item.id] for item in accounts if item.account_type == "asset"); liability_total = sum(balances[item.id] for item in accounts if item.account_type == "liability"); equity_total = sum(balances[item.id] for item in accounts if item.account_type == "equity"); profit = income_total - expense_total
    rows.extend([["集計", "損益", "", "当期利益", profit], ["集計", "貸借", "", "資産合計", asset_total], ["集計", "貸借", "", "負債・純資産・当期利益合計", liability_total + equity_total + profit], ["集計", "貸借", "", "貸借差額", asset_total - liability_total - equity_total - profit]])
    content = finance_export_csv(["帳票", "区分", "科目コード", "勘定科目・集計", "金額"], rows)
...
def finance_chart_account_stop(account_id: int, confirmed: bool = Form(False), access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    journal_line = session.scalar(select(FinanceJournalLine.id).where(FinanceJournalLine.tenant_id == tenant.id, FinanceJournalLine.account_id == account_id).limit(1))
    if not confirmed or not item or not item.active or item.system_key or mapped or active_sub or journal_line:
...
def finance_journal_voucher(session: Session, tenant_id: int, prefix: str, entry_day: date) -> str:
    for _ in range(10):
        value = f"{prefix}-{entry_day:%Y%m%d}-{secrets.token_hex(3).upper()}"
        if not session.scalar(select(FinanceJournalEntry.id).where(FinanceJournalEntry.tenant_id == tenant_id, FinanceJournalEntry.voucher_no == value)):
            return value
...
def finance_create_journal(session: Session, tenant_id: int, user_id: int, entry_day: date, description: str, voucher_no: str, lines: list[tuple[str, int, int | None, int, str]], source_entry_id: int | None = None, reversal_of_id: int | None = None, max_lines: int = 20) -> FinanceJournalEntry:
    ...
    subaccounts = {item.id: item for item in session.scalars(select(FinanceSubaccount).where(FinanceSubaccount.tenant_id == tenant_id, FinanceSubaccount.id.in_(subaccount_ids), FinanceSubaccount.active.is_(True))).all()} if subaccount_ids else {}
    if len(accounts) != len(account_ids) or len(subaccounts) != len(subaccount_ids) or any(sub_id and subaccounts[sub_id].account_id != account_id for _, account_id, sub_id, _, _ in lines):
        raise HTTPException(status_code=400, detail="勘定科目・補助科目を確認してください")
    journal = FinanceJournalEntry(tenant_id=tenant_id, entry_date=entry_day, voucher_no=voucher_no, description=description[:200], source_entry_id=source_entry_id, reversal_of_id=reversal_of_id, created_by_id=user_id)
    session.add(journal); session.flush()
    for line_no, (side, account_id, subaccount_id, amount, memo) in enumerate(lines, 1):
        session.add(FinanceJournalLine(tenant_id=tenant_id, journal_entry_id=journal.id, line_no=line_no, side=side, account_id=account_id, subaccount_id=subaccount_id, amount=amount, memo=memo[:200] or None))
    session.flush(); return journal


def finance_system_account(session: Session, tenant_id: int, system_key: str) -> FinanceChartAccount:
    account = session.scalar(select(FinanceChartAccount).where(FinanceChartAccount.tenant_id == tenant_id, FinanceChartAccount.system_key == system_key, FinanceChartAccount.active.is_(True)))
    if not account: raise HTTPException(status_code=409, detail="標準勘定科目を先に初期設定してください")
    return account


def finance_category_account(session: Session, tenant_id: int, entry_type: str, category: str) -> FinanceChartAccount:
    mapping = session.scalar(select(FinanceCategoryAccountMap).where(FinanceCategoryAccountMap.tenant_id == tenant_id, FinanceCategoryAccountMap.entry_type == entry_type, FinanceCategoryAccountMap.category == category))
    account = session.scalar(select(FinanceChartAccount).where(FinanceChartAccount.id == mapping.account_id, FinanceChartAccount.tenant_id == tenant_id, FinanceChartAccount.active.is_(True))) if mapping else None
    if not account: raise HTTPException(status_code=409, detail="費目に対応する勘定科目を設定してください")
    return account


def finance_create_invoice_accrual_journal(session: Session, tenant_id: int, user_id: int, invoice: Invoice) -> FinanceJournalEntry:
    existing = session.scalar(select(FinanceJournalEntry).where(FinanceJournalEntry.tenant_id == tenant_id, FinanceJournalEntry.voucher_no == f"AR-{invoice.id}"))
    if existing: return existing
    receivable = finance_system_account(session, tenant_id, "receivable"); sales = finance_category_account(session, tenant_id, "income", "sale")
    memo = f"請求書#{invoice.id} {invoice.invoice_no}"
    return finance_create_journal(session, tenant_id, user_id, invoice.issued_on, f"売掛金発生：{invoice.invoice_no}", f"AR-{invoice.id}", [("debit", receivable.id, None, invoice.amount, memo), ("credit", sales.id, None, invoice.amount, memo)])


def finance_create_payable_accrual_journal(session: Session, tenant_id: int, user_id: int, payable: FinancePayable) -> FinanceJournalEntry:
    existing = session.scalar(select(FinanceJournalEntry).where(FinanceJournalEntry.tenant_id == tenant_id, FinanceJournalEntry.voucher_no == f"AP-{payable.id}"))
    if existing: return existing
    expense = finance_category_account(session, tenant_id, "expense", payable.category); payable_account = finance_system_account(session, tenant_id, "payable")
    memo = f"買掛金#{payable.id} {payable.invoice_no or '番号なし'}"
    return finance_create_journal(session, tenant_id, user_id, payable.received_on, f"買掛金発生：{payable.description}", f"AP-{payable.id}", [("debit", expense.id, None, payable.amount, memo), ("credit", payable_account.id, None, payable.amount, memo)])


def finance_create_accrual_settlement_journal(session: Session, tenant_id: int, user_id: int, entry: FinancialEntry, amount: int, account_key: str, description: str, voucher_prefix: str) -> FinanceJournalEntry:
    ...
    if existing: return existing
    cash = finance_system_account(session, tenant_id, "cash"); accrual = finance_system_account(session, tenant_id, account_key)
    lines = [("debit", cash.id, None, amount, description), ("credit", accrual.id, None, amount, description)] if account_key == "receivable" else [("debit", accrual.id, None, amount, description), ("credit", cash.id, None, amount, description)]
...
def finance_journals_page(month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    user, tenant = access
    try: first_day = datetime.strptime(month, "%Y-%m").date().replace(day=1) if month else date.today().replace(day=1)
    except ValueError: raise HTTPException(status_code=400, detail="表示月を確認してください")
    if first_day < date(2000, 1, 1) or first_day > date(2100, 12, 1): raise HTTPException(status_code=400, detail="表示月を確認してください")
    last_day = (first_day.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(days=1)
    journals = session.scalars(select(FinanceJournalEntry).where(FinanceJournalEntry.tenant_id == tenant.id, FinanceJournalEntry.entry_date >= first_day, FinanceJournalEntry.entry_date <= last_day).order_by(FinanceJournalEntry.entry_date.desc(), FinanceJournalEntry.id.desc()).limit(1000)).all()
    journal_ids = [item.id for item in journals]
    lines = session.scalars(select(FinanceJournalLine).where(FinanceJournalLine.tenant_id == tenant.id, FinanceJournalLine.journal_entry_id.in_(journal_ids)).order_by(FinanceJournalLine.journal_entry_id, FinanceJournalLine.line_no).limit(20000)).all() if journal_ids else []
    accounts = session.scalars(select(FinanceChartAccount).where(FinanceChartAccount.tenant_id == tenant.id).order_by(FinanceChartAccount.code, FinanceChartAccount.id).limit(1000)).all()
    subaccounts = session.scalars(select(FinanceSubaccount).where(FinanceSubaccount.tenant_id == tenant.id, FinanceSubaccount.active.is_(True)).order_by(FinanceSubaccount.code, FinanceSubaccount.id).limit(2000)).all()
    account_names = {item.id: f"{item.code} {item.name}" for item in accounts}; sub_names = {item.id: item.name for item in subaccounts}; lines_by_journal: dict[int, list[FinanceJournalLine]] = {}
    for item in lines: lines_by_journal.setdefault(item.journal_entry_id, []).append(item)
    rows = ""; cards = ""
    for journal in journals:
        detail = "<br>".join(f'{"借" if line.side == "debit" else "貸"}：{html.escape(account_names.get(line.account_id, "科目不明"))}{"／" + html.escape(sub_names[line.subaccount_id]) if line.subaccount_id in sub_names else ""} ¥{line.amount:,}' for line in lines_by_journal.get(journal.id, []))
        reverse = f'<form class="inline" method="post" action="/modules/finance/journals/{journal.id}/reverse"><input type="date" name="reversed_on" value="{date.today()}" min="{journal.entry_date}" max="{date.today()}" required><label><input type="checkbox" name="confirmed" value="true" style="width:auto" required> 取消確認</label><button class="danger">取消仕訳</button></form>' if journal.status == "posted" else ""
        rows += f'<tr><td>{journal.entry_date}</td><td>{html.escape(journal.voucher_no)}</td><td>{html.escape(journal.description)}</td><td>{detail}</td><td><span class="badge">{"計上済み" if journal.status == "posted" else "取消済み"}</span>{reverse}</td></tr>'
        cards += f'<article class="calendar-mobile-card"><h3>{html.escape(journal.voucher_no)}／{html.escape(journal.description)}</h3><p>{journal.entry_date}／<span class="badge">{"計上済み" if journal.status == "posted" else "取消済み"}</span></p><p>{detail}</p>{reverse}</article>'
    ...
    <h2>仕訳伝票（{first_day}～{last_day}）</h2><div class="calendar-desktop-only" style="overflow-x:auto"><table><tr><th>日付</th><th>伝票番号</th><th>摘要</th><th>仕訳明細</th><th>状態・操作</th></tr>{rows or '<tr><td colspan="5">対象月の仕訳はありません。</td></tr>'}</table></div><section class="calendar-mobile-only">{cards or '<div class="tenant">対象月の仕訳はありません。</div>'}</section>'''
    return layout("複式簿記仕訳", body, user)


@app.post("/modules/finance/journals")
def finance_journal_create(entry_date: str = Form(...), description: str = Form(...), amount: int = Form(...), debit_account_id: int = Form(...), credit_account_id: int = Form(...), debit_subaccount_id: int | None = Form(None), credit_subaccount_id: int | None = Form(None), memo: str = Form(""), confirmed: bool = Form(False), access=Depends(require_tenant_admin), session: Session = Depends(db)):
    user, tenant = access; clean_description = description.strip()
    try: entry_day = date.fromisoformat(entry_date)
    except ValueError: raise HTTPException(status_code=400, detail="仕訳日を確認してください")
    if not confirmed or entry_day < date(2000, 1, 1) or entry_day > date.today() or not clean_description or len(clean_description) > 200 or amount <= 0 or amount > 999999999 or len(memo) > 200:
        raise HTTPException(status_code=400, detail="仕訳内容を確認してください")
    ensure_finance_period_open(session, tenant.id, entry_day)
    journal = finance_create_journal(session, tenant.id, user.id, entry_day, clean_description, finance_journal_voucher(session, tenant.id, "JV", entry_day), [("debit", debit_account_id, debit_subaccount_id, amount, memo), ("credit", credit_account_id, credit_subaccount_id, amount, memo)])
    record_finance_audit(session, tenant.id, user.id, "journal_create", "finance_journal_entry", journal.id, "複式仕訳を登録", f"voucher={journal.voucher_no} amount={amount}")
    session.commit(); return RedirectResponse(f"/modules/finance/journals?month={entry_day:%Y-%m}", status_code=303)


@app.post("/modules/finance/journals/sync")
def finance_journals_sync(confirmed: bool = Form(False), access=Depends(require_tenant_admin), session: Session = Depends(db)):
    user, tenant = access
    if not confirmed: raise HTTPException(status_code=400, detail="収支連携を確認してください")
    cash = session.scalar(select(FinanceChartAccount).where(FinanceChartAccount.tenant_id == tenant.id, FinanceChartAccount.system_key == "cash", FinanceChartAccount.active.is_(True)))
    mappings = session.scalars(select(FinanceCategoryAccountMap).where(FinanceCategoryAccountMap.tenant_id == tenant.id)).all(); mapping_by_key = {(item.entry_type, item.category): item.account_id for item in mappings}
    linked_ids = set(session.scalars(select(FinanceJournalEntry.source_entry_id).where(FinanceJournalEntry.tenant_id == tenant.id, FinanceJournalEntry.source_entry_id.is_not(None))).all())
    ...
    for item in eligible:
        ...
        lines = [("debit", cash.id, None, item.amount, "収支台帳連携"), ("credit", mapped_id, None, item.amount, "収支台帳連携")] if item.entry_type == "income" else [("debit", mapped_id, None, item.amount, "収支台帳連携"), ("credit", cash.id, None, item.amount, "収支台帳連携")]
        finance_create_journal(session, tenant.id, user.id, item.occurred_on, item.description, f"LG-{item.id}", lines, source_entry_id=item.id)
    record_finance_audit(session, tenant.id, user.id, "journal_sync", "finance_journal_entry", None, "収支台帳を複式仕訳へ連携", f"count={len(eligible)}")
    session.commit(); return RedirectResponse("/modules/finance/journals", status_code=303)


@app.post("/modules/finance/journals/{journal_id}/reverse")
def finance_journal_reverse(journal_id: int, reversed_on: str = Form(...), confirmed: bool = Form(False), access=Depends(require_tenant_admin), session: Session = Depends(db)):
    user, tenant = access; original = session.scalar(select(FinanceJournalEntry).where(FinanceJournalEntry.id == journal_id, FinanceJournalEntry.tenant_id == tenant.id).with_for_update())
    try: reversed_day = date.fromisoformat(reversed_on)
    except ValueError: raise HTTPException(status_code=400, detail="取消日を確認してください")
    existing = session.scalar(select(FinanceJournalEntry.id).where(FinanceJournalEntry.tenant_id == tenant.id, FinanceJournalEntry.reversal_of_id == journal_id))
    if not confirmed or not original or original.status != "posted" or original.reversal_of_id is not None or existing or reversed_day < original.entry_date or reversed_day > date.today(): raise HTTPException(status_code=400, detail="取消仕訳の内容を確認してください")
    ensure_finance_period_open(session, tenant.id, reversed_day)
    original_lines = session.scalars(select(FinanceJournalLine).where(FinanceJournalLine.tenant_id == tenant.id, FinanceJournalLine.journal_entry_id == original.id).order_by(FinanceJournalLine.line_no)).all()
    reversed_lines = [("credit" if item.side == "debit" else "debit", item.account_id, item.subaccount_id, item.amount, f"取消：{item.memo or original.description}") for item in original_lines]
    reversal = finance_create_journal(session, tenant.id, user.id, reversed_day, f"取消：{original.description}"[:200], finance_journal_voucher(session, tenant.id, "RV", reversed_day), reversed_lines, reversal_of_id=original.id)
    original.status = "reversed"; record_finance_audit(session, tenant.id, user.id, "journal_reverse", "finance_journal_entry", original.id, "複式仕訳を取消", f"reversal={reversal.id} voucher={reversal.voucher_no}")
    session.commit(); return RedirectResponse(f"/modules/finance/journals?month={reversed_day:%Y-%m}", status_code=303)


@app.get("/modules/finance/journals.csv")
def finance_journals_csv(month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    _, tenant = access
    try: first_day = datetime.strptime(month, "%Y-%m").date().replace(day=1) if month else date.today().replace(day=1)
    except ValueError: raise HTTPException(status_code=400, detail="表示月を確認してください")
    if first_day < date(2000, 1, 1) or first_day > date(2100, 12, 1): raise HTTPException(status_code=400, detail="表示月を確認してください")
    last_day = (first_day.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(days=1)
    journals = session.scalars(select(FinanceJournalEntry).where(FinanceJournalEntry.tenant_id == tenant.id, FinanceJournalEntry.entry_date >= first_day, FinanceJournalEntry.entry_date <= last_day).order_by(FinanceJournalEntry.entry_date, FinanceJournalEntry.id).limit(1000)).all(); journal_ids = [item.id for item in journals]
    lines = session.scalars(select(FinanceJournalLine).where(FinanceJournalLine.tenant_id == tenant.id, FinanceJournalLine.journal_entry_id.in_(journal_ids)).order_by(FinanceJournalLine.journal_entry_id, FinanceJournalLine.line_no).limit(20000)).all() if journal_ids else []
...
def finance_payable_create(vendor_id: int = Form(...), received_on: str = Form(...), due_on: str = Form(...), category: str = Form(...), amount: int = Form(...), description: str = Form(...), invoice_no: str = Form(""), notes: str = Form(""), access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    payable = FinancePayable(tenant_id=tenant.id, vendor_id=vendor.id, received_on=received_day, due_on=due_day, category=category, description=clean_description, invoice_no=invoice_no.strip() or None, amount=amount, status="unpaid", notes=notes.strip() or None, created_by_id=user.id)
    session.add(payable); session.flush(); journal = finance_create_payable_accrual_journal(session, tenant.id, user.id, payable)
    record_finance_audit(session, tenant.id, user.id, "payable_accrual", "finance_payable", payable.id, "買掛金を発生主義で計上", f"journal={journal.id} amount={amount}")
    session.commit()
    return RedirectResponse("/modules/finance/payables", status_code=303)
...
def finance_payable_pay(payable_id: int, paid_on: str = Form(...), account_id: int = Form(...), confirmed: bool = Form(False), access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    finance_create_payable_accrual_journal(session, tenant.id, user.id, payable)
    ...
    journal = finance_create_accrual_settlement_journal(session, tenant.id, user.id, entry, payable.amount, "payable", f"買掛金支払：{payable.description}", "AP-PAY")
    payable.status = "paid"; payable.paid_on = payment_day; payable.account_id = account.id; payable.financial_entry_id = entry.id
    record_finance_audit(session, tenant.id, user.id, "payable_payment", "finance_payable", payable.id, "買掛金を支払済みに更新", f"ledger={entry.id} journal={journal.id} account={account.id} amount={payable.amount}")
    session.commit()
    return RedirectResponse("/modules/finance/payables", status_code=303)
...
def finance_payable_cancel(payable_id: int, confirmed: bool = Form(False), access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    payable = session.scalar(select(FinancePayable).where(FinancePayable.id == payable_id, FinancePayable.tenant_id == tenant.id).with_for_update())
    ...
    original = finance_create_payable_accrual_journal(session, tenant.id, user.id, payable)
    reversal = finance_reverse_accrual_journal(session, tenant.id, user.id, original, date.today(), f"買掛金取消：{payable.description}")
    payable.status = "cancelled"; record_finance_audit(session, tenant.id, user.id, "payable_accrual_reverse", "finance_payable", payable.id, "買掛金の取消仕訳を計上", f"journal={reversal.id} amount={payable.amount}"); session.commit()
    return RedirectResponse("/modules/finance/payables", status_code=303)
...
def settle_invoice_receivable(session: Session, tenant_id: int, user_id: int, invoice: Invoice, account: FinanceAccount, received_on: date, statement_line: FinanceStatementLine | None = None) -> FinanceReceivableSettlement:
    finance_create_invoice_accrual_journal(session, tenant_id, user_id, invoice)
    ...
    finance_create_accrual_settlement_journal(session, tenant_id, user_id, entry, invoice.amount, "receivable", f"売掛金回収：{invoice.invoice_no}", "AR-RCV")
...
def generate_due_finance_recurring(session: Session, target_day: date) -> int:
    ...
    for rule in rules:
        ...
        if exists:
            continue
        if finance_period_close(session, rule.tenant_id, posting_day):
            continue
...
@app.get("/modules/finance/reconciliation", response_class=HTMLResponse)
def finance_reconciliation_page(as_of: str = "", access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    try:
        target_day = date.fromisoformat(as_of) if as_of else date.today()
    except ValueError:
        raise HTTPException(status_code=400, detail="照合日を確認してください")
    if target_day < date(2000, 1, 1) or target_day > date.today():
        raise HTTPException(status_code=400, detail="照合日を確認してください")
    accounts = session.scalars(select(FinanceAccount).where(FinanceAccount.tenant_id == tenant.id).order_by(FinanceAccount.active.desc(), FinanceAccount.id)).all()
    active_accounts = [item for item in accounts if item.active]
    ...
    for item in histories:
        latest_by_account.setdefault(item.account_id, item)
    summary_cards = ""
    for account in accounts:
...
@app.post("/modules/finance/reconciliation")
def finance_reconciliation_save(statement_on: str = Form(...), account_id: int = Form(...), actual_balance: int = Form(...), notes: str = Form(""), access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    try:
        target_day = date.fromisoformat(statement_on)
    except ValueError:
        raise HTTPException(status_code=400, detail="照合日を確認してください")
    account = session.scalar(select(FinanceAccount).where(FinanceAccount.id == account_id, FinanceAccount.tenant_id == tenant.id, FinanceAccount.active.is_(True)))
    if not account or target_day < date(2000, 1, 1) or target_day > date.today() or actual_balance < -999999999 or actual_balance > 999999999 or len(notes) > 500:
        raise HTTPException(status_code=400, detail="残高照合の内容を確認してください")
    ledger_balance = finance_account_balance_on(session, tenant.id, account, target_day)
...
def finance_export_page(access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    <div class="tenant"><strong>ZIPに含まれる内容</strong><ul><li>収支・経費台帳CSV</li><li>請求書CSV</li><li>経費申請・承認履歴CSV</li><li>原価配賦CSV</li><li>領収書・証憑原本と一覧CSV</li><li>全ファイルのSHA-256整合性情報</li></ul></div>
...
def invoice_status_update(invoice_id: int, status_value: str = Form(..., alias="status"), access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    invoice = session.scalar(select(Invoice).where(Invoice.id == invoice_id, Invoice.tenant_id == tenant.id).with_for_update())
    ...
    if invoice.status == "draft" and status_value == "issued":
        ensure_finance_period_open(session, tenant.id, invoice.issued_on)
        journal = finance_create_invoice_accrual_journal(session, tenant.id, user.id, invoice)
        record_finance_audit(session, tenant.id, user.id, "receivable_accrual", "invoice", invoice.id, "売掛金を発生主義で計上", f"journal={journal.id} amount={invoice.amount}")
    elif invoice.status == "issued" and status_value == "cancelled":
        original = finance_create_invoice_accrual_journal(session, tenant.id, user.id, invoice)
        reversal = finance_reverse_accrual_journal(session, tenant.id, user.id, original, date.today(), f"売掛金取消：{invoice.invoice_no}")
        record_finance_audit(session, tenant.id, user.id, "receivable_accrual_reverse", "invoice", invoice.id, "売掛金の取消仕訳を計上", f"journal={reversal.id} amount={invoice.amount}")
    elif invoice.status == "issued" and status_value != "issued":
        raise HTTPException(status_code=400, detail="発行済み請求書は入金消込または取消で処理してください")
...
FINANCE_DOCUMENT_TYPES = {"receipt": "領収書", "invoice": "請求書", "statement": "明細書", "other": "その他"}


@app.get("/modules/finance/documents", response_class=HTMLResponse)
def finance_documents_page(document_type: str = "", document_keyword: str = "", access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    if document_type not in {"", *FINANCE_DOCUMENT_TYPES} or len(document_keyword) > 100:
        raise HTTPException(status_code=400, detail="検索条件を確認してください")
...
@app.post("/modules/finance/documents")
...
@app.get("/modules/finance/documents/{document_id}/file")
def finance_document_file(document_id: int, access=Depends(require_tenant_user), session: Session = Depends(db)):
    user, tenant = access
    item = session.scalar(select(FinanceDocument).where(FinanceDocument.id == document_id, FinanceDocument.tenant_id == tenant.id))
    if not item: raise HTTPException(status_code=404, detail="書類が見つかりません")
    return Response(content=item.file_data, media_type=item.content_type, headers={"Cache-Control": "private, no-store", "X-Content-Type-Options": "nosniff", "Content-Disposition": f"inline; filename*=UTF-8''{quote(item.filename)}"})
...
async def line_webhook(webhook_key: str, request: Request, x_line_signature: str | None = Header(None)):
    with SessionLocal() as session:
        ...
        # LINE Developersの「検証」はeventsが空のため、受信記録を先に確定する。
        session.commit()
...
def family_dog_health_calendar(dog_id: int, month: str = "", user: User = Depends(require_user), session: Session = Depends(db)):
    ...
    mobile_calendar_items = "".join(
        f'''<article class="health-mobile-card"><h3>{day.month}月{day.day}日（{weekday_labels[day.weekday()]}）</h3><p><span class="badge" style="background:{colors[category]}">{label}</span> <a href="/family/dogs/{dog.id}/health/{category}">{html.escape(title)}</a></p>
        <form method="post" action="/family/dogs/{dog.id}/health/schedules/complete"><input type="hidden" name="category" value="{category}"><input type="hidden" name="title" value="{html.escape(title)}"><input type="hidden" name="due_on" value="{day}"><input type="hidden" name="return_month" value="{first_day:%Y-%m}"><button class="success">実施済みにする</button></form></article>'''
        for day in sorted(events) for category, label, title in events[day]
    )
    body = f'''<a class="button secondary" href="/family/dogs/{dog.id}/health">健康管理へ戻る</a>
    <h1>{html.escape(dog.call_name)}の健康カレンダー</h1>
    <div class="health-month-nav" style="display:flex;align-items:center;justify-content:space-between;gap:12px"><a class="button secondary" href="?month={previous_month:%Y-%m}">← 前月</a><h2>{first_day.year}年{first_day.month}月</h2><a class="button secondary" href="?month={next_month:%Y-%m}">翌月 →</a></div>
    ...
    <div class="health-desktop-only" style="overflow-x:auto"><table style="table-layout:fixed;min-width:800px"><tr><th style="color:#b54b56">日</th><th>月</th><th>火</th><th>水</th><th>木</th><th>金</th><th style="color:#416b9b">土</th></tr>{cells}</table></div>
    <section class="health-mobile-only" aria-label="今月の健康予定">{mobile_calendar_items or '<div class="tenant">今月の健康予定はありません。</div>'}</section>
...
def family_health_schedule_completion_history(dog_id: int, user: User = Depends(require_user), session: Session = Depends(db)):
    ...
    for item in completions:
        ...
        mobile_cards += f'''<article class="health-mobile-card"><h3>{labels.get(item.category, "健康予定")}：{html.escape(item.title)}</h3><p>予定日：{item.due_on}</p><p>完了操作：{completed_at.strftime("%Y-%m-%d %H:%M")}</p><form method="post" action="/family/dogs/{dog.id}/health/schedules/completed/{item.id}/undo"><label style="font-weight:400"><input type="checkbox" name="confirm_undo" value="true" style="width:auto" required> 取り消しを確認</label><button class="secondary">未完了に戻す</button></form></article>'''
    body = f'''<a class="button secondary" href="/family/dogs/{dog.id}/health">健康管理へ戻る</a>
    <h1>{html.escape(dog.call_name)}の実施済み健康予定</h1>
    <p>実施済みにした健康予定を新しい順に表示します。取り消すと、対象期間の通知・健康トップ・カレンダーへ再表示されます。</p>
    <div class="health-desktop-only" style="overflow-x:auto"><table><tr><th>予定日</th><th>種類</th><th>内容</th><th>完了操作日時</th><th>操作</th></tr>{rows or '<tr><td colspan="5">実施済みの健康予定はありません。</td></tr>'}</table></div><section class="health-mobile-only">{mobile_cards or '<div class="tenant">実施済みの健康予定はありません。</div>'}</section>'''
...
def family_dog_health(dog_id: int, health_category: str = "", date_from: str = "", date_to: str = "", keyword: str = "", user: User = Depends(require_user), session: Session = Depends(db)):
    ...
    due_mobile_cards = "".join(f'''<article class="health-mobile-card"><h3>{label}：<a href="/family/dogs/{dog.id}/health/{category}">{html.escape(title)}</a></h3><p>予定日：{due}　<span class="badge" style="{'background:#f4c9ca;color:#8d3037' if days < 0 else 'background:#f6e1b8;color:#755514'}">{abs(days)}日{'超過' if days < 0 else '後'}</span></p><form method="post" action="/family/dogs/{dog.id}/health/schedules/complete"><input type="hidden" name="category" value="{category}"><input type="hidden" name="title" value="{html.escape(title)}"><input type="hidden" name="due_on" value="{due}"><button class="success">実施済みにする</button></form></article>''' for label, title, due, days, category in due_items)
    dashboard = f'''<h2>健康サマリー</h2><div class="grid"><section class="tenant"><h3>最新体重</h3><strong>{f'{latest_weight[1]:g}kg' if latest_weight else '未登録'}</strong><p>{latest_weight[0] if latest_weight else ''}</p></section><section class="tenant"><h3>継続中の投薬</h3><strong>{active_medications}件</strong></section><section class="tenant"><h3>治療・観察・慢性</h3><strong>{active_diseases}件</strong></section><section class="tenant"><h3>現在のフード</h3><strong>{len(active_food_names)}件</strong><p>{html.escape('、'.join(active_food_names) or '未登録')}</p></section></div><div class="grid"><section class="tenant"><h3>30日以内の予定</h3><strong>{upcoming_count}件</strong></section><section class="tenant"><h3>期限超過</h3><strong class="{'error' if overdue_count else ''}">{overdue_count}件</strong></section></div><h2>これからの健康予定</h2><div class="health-desktop-only" style="overflow-x:auto"><table><tr><th>種類</th><th>内容</th><th>予定日</th><th>状態</th><th>操作</th></tr>{due_rows or '<tr><td colspan="5">30日以内または期限超過の予定はありません。</td></tr>'}</table></div><section class="health-mobile-only">{due_mobile_cards or '<div class="tenant">30日以内または期限超過の予定はありません。</div>'}</section>'''
    body = f'''<div class="health-toolbar"><a class="button secondary" href="/family/dogs/{dog.id}">{html.escape(dog.call_name)}のページへ戻る</a><a class="button" href="/family/dogs/{dog.id}/health/calendar">健康カレンダー</a><a class="button secondary" href="/family/dogs/{dog.id}/health/schedules/completed">実施済み履歴</a><a class="button" href="{report_url}">表示条件でPDF出力</a><a class="button secondary" href="{csv_url}">表示条件でCSV出力</a></div>
