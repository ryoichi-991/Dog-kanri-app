...
def dashboard_priority_items(tenant_id: int, session: Session) -> list[tuple[date, str, str, str]]:
    ...
    for task in session.scalars(select(TaskEvent).where(TaskEvent.tenant_id == tenant_id, TaskEvent.completed.is_(False), TaskEvent.due_date <= limit_day)).all():
        if task.category == "breeding" and (task.dog_id, task.due_date) in delivered_birth_todo_keys and task.title.endswith(" 出産予定"):
            continue
...
def todo_page(access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    tasks = [task for task in tasks if not (task.category == "breeding" and (task.dog_id, task.due_date) in delivered_birth_todo_keys and task.title.endswith(" 出産予定"))]
...
def calendar_page(month: str = "", calendar_category: str = "", calendar_state: str = "", show_all: bool = False, access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    for item in session.scalars(select(BreedingRecord).where(BreedingRecord.tenant_id == tenant.id)).all():
        ...
        while candidate_day <= candidate_end:
            ...
            add_event(candidate_day, title, "breeding", "交配記録", "/modules/births")
            candidate_day += timedelta(days=1)
    for item in litters:
        dog = dogs.get(item.dam_id)
        add_event(item.birth_date, f"{dog.call_name if dog else '母犬'} 出産", "breeding", "出産記録", "/modules/births", True)
...
def finance_trial_balance_csv(start_year: str = "", through_month: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    content = finance_export_csv(["種別", "区分", "科目コード", "勘定科目・集計", "期首借方", "期首貸�
... 235795 bytes omitted ...
er)
