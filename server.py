...
MODULES = {
    ...
    "finance/books": ("仕訳帳・科目別元帳", "複式仕訳の借方・貸方、勘定科目・補助科目別帳簿とCSV出力"),
...
def page_usage_guide(title: str) -> str:
    ...
    guides = [
        ...
        (("仕訳帳", "科目別元帳", "会計帳簿"), ["複式仕訳を伝票番号順の仕訳帳と勘定科目別集計で確認できます。", "借方・貸方、補助科目、期首残高、発生主義、取消仕訳を一体で確認できます。"], ["対象年・月、損益区分、費目対応、勘定科目を選びます。", "借方合計・貸方合計と明細を照合します。", "税理士共有や保管が必要な場合はCSVを出力します。"], "複式仕訳を基礎にした会計帳簿です。正式な決算・税務申告では税理士と勘定科目・補助科目・期首残高を確認してください。"),
        ...
        (("収支", "経費", "原価", "請求"), ["犬舎ごとの入金・経費を収支台帳と複式仕訳へ同時に記録できます。", "現金基準の台帳収支と、売掛・買掛・減価償却・取消を含む発生主義の会計利益を月別に比較できます。", "各台帳記録の伝票番号と仕訳済み・未連携状態を確認できます。"], ["表示月と区分を選び、台帳実績と複式仕訳実績を比較します。", "入金または経費を登録し、借方・貸方の伝票が作成されたことを確認します。", "仕訳未連携がある場合は複式簿記仕訳画面で補完し、総勘定元帳で内訳を照合します。"], "この画面だけで税務申告用の会計帳簿が完成するものではなく、台帳収支と発生主義の会計利益は集計基準が異なるため一致しない場合があります。領収書・請求書の原本と照合し、正式な決算・税務申告は税理士へ確認してください。"),
...
def litter_delete(litter_id: int, access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    if session.scalar(select(CostAllocation.id).where(CostAllocation.tenant_id == tenant.id, CostAllocation.litter_id == litter.id).limit(1)):
        raise HTTPException(status_code=400, detail="原価配賦で使用中のため削除できません。先に会計記録を確認してください")
...
def categorized_dogs_page(category: str, access=Depends(require_tenant_user), session: Session = Depends(db)):
    ...
    for dog in dogs:
        ...
        search_text = " ".join(str(value) for value in (
            dog.call_name, dog.registered_name, dog.breed, dog.pedigree_no,
            dog.origin_registration_no, dog.microchip_no, dog.color,
            "牡" if dog.sex == "male" else "牝",
            sire.registered_name or sire.call_name if sire else "",
            dam.registered_name or dam.call_name if dam else "",
            buyer_name if category == "puppy" else "",
        ) if value).lower()
        rows += f'''<tr class="categorized-dog-row" data-search="{html.escape(search_text)}"><td><a href="/modules/dogs/{dog.id}"><strong>{html.escape(dog.call_name)}</strong></a><br><small>{html.escape(dog.registered_name or "血統名未登録")}</small></td><td>{title_marks(dog.titles) or "-"}</td><td>{"牡" if dog.sex == "male" else "牝"}</td><td>{html.escape(dog.breed or "-")}</td><td>{dog.birth_date or "-"}</td><td>{html.escape(dog.color or "-")}</td><td>{html.escape(dog.pedigree_no or "-")}</td><td>{html.escape(sire.registered_name or sire.call_name) if sire else "-"}</td><td>{html.escape(dam.registered_name or dam.call_name) if dam else "-"}</td><td><span class="badge">{status_labels.get(dog.status, dog.status)}</span></td>{f'<td>{html.escape(buyer_name)}</td>' if category == 'puppy' else ''}<td><a class="button secondary" href="/modules/dogs/{dog.id}/edit">編集</a></td></tr>'''
    ...
    body = f'''<h1>{labels[category]}</h1><p>{html.escape(tenant.name)} — {description}</p>{metrics}<div class="tenant"><label for="categorized-dog-search">登録犬を検索</label><input id="categorized-dog-search" type="search" placeholder="呼び名・血統書名・犬種・登録番号・マイクロチップ番号など" autocomplete="off"><p id="categorized-dog-result" style="margin:6px 0 0;color:#765f68;font-size:12px"></p></div><table><thead><tr><th>犬名</th><th>タイトル</th><th>性別</th><th>犬種</th><th>生年月日</th><th>毛色</th><th>血統書番号</th><th>父犬</th><th>母犬</th><th>状態</th>{buyer_header}<th>操作</th></tr></thead><tbody>{rows}<tr id="categorized-dog-empty" style="display:none"><td colspan="{columns}">検索条件に一致する犬はいません。</td></tr>{'' if rows else f'<tr class="categorized-dog-initial-empty"><td colspan="{columns}">登録犬はいません。</td></tr>'}</tbody></table><script>(function(){{const search=document.getElementById('categorized-dog-search');const result=document.getElementById('categorized-dog-result');const rows=Array.from(document.querySelectorAll('.categorized-dog-row'));const empty=document.getElementById('categorized-dog-empty');function render(){{const keyword=search.value.trim().toLocaleLowerCase('ja');let count=0;for(const row of rows){{const visible=!keyword||row.dataset.search.toLocaleLowerCase('ja').includes(keyword);row.style.display=visible?'':'none';if(visible)count++;}}empty.style.display=keyword&&count===0?'':'none';result.textContent=keyword?count+'頭が見つかりました':rows.length+'頭から検索できます';}}search.addEventListener('input',render);render();}})();</script>'''
...
def finance_double_entry_book_data(session: Session, tenant_id: int, year: int, month: int, entry_type: str, category: str, account_id: int):
    first_day = date(year, month or 1, 1)
    last_day = ((first_day.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(days=1)) if month else date(year, 12, 31)
    accounts, subaccounts, journals, lines = finance_double_entry_data(session, tenant_id, first_day, last_day)
    account_by_id = {item.id: item for item in accounts}; subaccount_by_id = {item.id: item for item in subaccounts}; journal_by_id = {item.id: item for item in journals}
    if account_id and account_id not in account_by_id:
        raise HTTPException(status_code=400, detail="対象勘定科目を確認してください")
    mapped_account_ids = set(session.scalars(select(FinanceCategoryAccountMap.account_id).where(FinanceCategoryAccountMap.tenant_id == tenant_id, FinanceCategoryAccountMap.category == category)).all()) if category else set()
    account_type = "revenue" if entry_type == "income" else ("expense" if entry_type == "expense" else "")
    filtered_lines = [line for line in lines if line.account_id in account_by_id and (not account_id or line.account_id == account_id) and (not account_type or account_by_id[line.account_id].account_type == account_type) and (not category or line.account_id in mapped_account_ids)]
    return first_day, last_day, accounts, account_by_id, subaccount_by_id, journal_by_id, filtered_lines
...
def finance_books_page(year: str = "", month: str = "", entry_type: str = "", category: str = "", account_id: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    first_day, last_day, accounts, account_by_id, subaccount_by_id, journal_by_id, lines = finance_double_entry_book_data(session, tenant.id, selected_year, selected_month, entry_type, category, selected_account_id)
    debit_total = sum(item.amount for item in lines if item.side == "debit"); credit_total = sum(item.amount for item in lines if item.side == "credit")
    account_totals: dict[int, dict[str, int]] = {}
    for line in lines:
        totals = account_totals.setdefault(line.account_id, {"debit": 0, "credit": 0}); totals[line.side] += line.amount
    rows = "".join(f'<tr><td>{journal_by_id[line.journal_entry_id].entry_date}</td><td>{html.escape(journal_by_id[line.journal_entry_id].voucher_no)}</td><td>{line.line_no}</td><td>{html.escape(account_by_id[line.account_id].code)} {html.escape(account_by_id[line.account_id].name)}</td><td>{html.escape(subaccount_by_id[line.subaccount_id].name) if line.subaccount_id in subaccount_by_id else "－"}</td><td>{html.escape(journal_by_id[line.journal_entry_id].description)}</td><td>{f"¥{line.amount:,}" if line.side == "debit" else "－"}</td><td>{f"¥{line.amount:,}" if line.side == "credit" else "－"}</td><td>{html.escape(line.memo or "－")}</td></tr>' for line in lines)
    cards = "".join(f'<article class="calendar-mobile-card"><h3>{html.escape(journal_by_id[line.journal_entry_id].voucher_no)}／{html.escape(account_by_id[line.account_id].name)}</h3><p>{journal_by_id[line.journal_entry_id].entry_date}／{line.line_no}行／{html.escape(subaccount_by_id[line.subaccount_id].name) if line.subaccount_id in subaccount_by_id else "補助科目なし"}</p><p>{"借方" if line.side == "debit" else "貸方"} <strong>¥{line.amount:,}</strong>／{html.escape(journal_by_id[line.journal_entry_id].description)}</p></article>' for line in lines)
    ledger_rows = "".join(f'<tr><td>{html.escape(account_by_id[key].code)}</td><td>{html.escape(account_by_id[key].name)}</td><td>¥{value["debit"]:,}</td><td>¥{value["credit"]:,}</td><td class="{"error" if value["debit"] - value["credit"] < 0 else ""}">{value["debit"] - value["credit"]:,}</td></tr>' for key, value in sorted(account_totals.items(), key=lambda pair: account_by_id[pair[0]].code))
    ...
    account_options = "".join(f'<option value="{item.id}" {"selected" if selected_account_id == item.id else ""}>{html.escape(item.code)} {html.escape(item.name)}</option>' for item in accounts)
    ...
    body = f'''<h1>仕訳帳・科目別元帳</h1><p>複式仕訳を日付・伝票番号・借方・貸方・勘定科目・補助科目別に確認し、税理士共有用CSVを出力します。</p>
    <form method="get"><div class="grid"><div><label>年</label><input type="number" name="year" min="2000" max="2100" value="{selected_year}" required></div><div><label>月</label><select name="month">{month_options}</select></div><div><label>損益区分</label><select name="entry_type">{type_options}</select></div><div><label>費目対応</label><select name="category">{category_options}</select></div><div><label>勘定科目</label><select name="account_id"><option value="">すべて</option>{account_options}</select></div></div><button>帳簿を表示</button> <a class="button secondary" href="/modules/finance/books.csv?{query_string}">CSV出力</a></form>
    <div class="grid"><div class="module"><h3>借方合計</h3><strong>¥{debit_total:,}</strong></div><div class="module"><h3>貸方合計</h3><strong>¥{credit_total:,}</strong></div><div class="module"><h3>貸借差額</h3><strong class="{'error' if debit_total != credit_total and not selected_account_id and not entry_type and not category else ''}">¥{debit_total-credit_total:,}</strong></div><div class="module"><h3>仕訳明細</h3><strong>{len(lines)}件</strong></div></div>
    <p class="tenant">期首残高・年度繰越・発生主義・決済・取消仕訳を含む複式仕訳帳です。正式な決算・税務申告は税理士へご確認ください。</p>
    <h2>仕訳帳（{first_day}～{last_day}）</h2><div class="calendar-desktop-only" style="overflow-x:auto"><table><tr><th>日付</th><th>伝票番号</th><th>行</th><th>勘定科目</th><th>補助科目</th><th>摘要</th><th>借方</th><th>貸方</th><th>メモ</th></tr>{rows or '<tr><td colspan="9">条件に一致する複式仕訳はありません。</td></tr>'}</table></div><section class="calendar-mobile-only">{cards or '<div class="tenant">条件に一致する複式仕訳はありません。</div>'}</section>
    <h2>科目別元帳集計</h2><table><tr><th>コード</th><th>勘定科目</th><th>借方合計</th><th>貸方合計</th><th>差額</th></tr>{ledger_rows or '<tr><td colspan="5">条件に一致する複式仕訳はありません。</td></tr>'}</table><div class="health-toolbar"><a class="button secondary" href="/modules/finance/general-ledger">総勘定元帳・複式試算表</a><a class="button secondary" href="/modules/finance/journals">複式仕訳を登録</a></div>'''
...
def finance_books_csv(year: str = "", month: str = "", entry_type: str = "", category: str = "", account_id: str = "", access=Depends(require_tenant_admin), session: Session = Depends(db)):
    _, tenant = access
    selected_year, selected_month, entry_type, category, selected_account_id = finance_book_filters(year, month, entry_type, category, account_id)
    _, _, _, account_by_id, subaccount_by_id, journal_by_id, lines = finance_double_entry_book_data(session, tenant.id, selected_year, selected_month, entry_type, category, selected_account_id)
    book_rows = [[journal_by_id[line.journal_entry_id].entry_date, journal_by_id[line.journal_entry_id].voucher_no, line.line_no, account_by_id[line.account_id].code, account_by_id[line.account_id].name, subaccount_by_id[line.subaccount_id].code if line.subaccount_id in subaccount_by_id else "", subaccount_by_id[line.subaccount_id].name if line.subaccount_id in subaccount_by_id else "", journal_by_id[line.journal_entry_id].description, line.amount if line.side == "debit" else "", line.amount if line.side == "credit" else "", line.memo or "", journal_by_id[line.journal_entry_id].status] for line in lines]
    content = finance_export_csv(["日付", "伝票番号", "行番号", "勘定科目コード", "勘定科目", "補助科目コード", "補助科目", "摘要", "借方金額", "貸方金額", "メモ", "状態"], book_rows)
...
def finance_export_page(access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    <div class="tenant"><strong>ZIPに含まれる内容</strong><ul><li>複式仕訳伝票・借方貸方明細CSV</li><li>勘定科目・補助科目・費目対応CSV</li><li>収支・経費台帳、請求書、経費申請・承認、原価配賦CSV</li><li>領収書・証憑原本と一覧CSV</li><li>全ファイルのSHA-256整合性情報</li></ul></div>
...
def finance_export_download(year: int = Form(...), admin_password: str = Form(...), confirmed: bool = Form(False), access=Depends(require_tenant_admin), session: Session = Depends(db)):
    ...
    chart_accounts, subaccounts, journals, journal_lines = finance_double_entry_data(session, tenant.id, year_start, year_end)
    category_maps = session.scalars(select(FinanceCategoryAccountMap).where(FinanceCategoryAccountMap.tenant_id == tenant.id).order_by(FinanceCategoryAccountMap.entry_type, FinanceCategoryAccountMap.category).limit(1000)).all()
    ...
    account_map = {item.id: item for item in chart_accounts}; subaccount_map = {item.id: item for item in subaccounts}; journal_map = {item.id: item for item in journals}
    chart_accounts_csv = finance_export_csv(["ID", "コード", "勘定科目", "区分", "通常残高", "システムキー", "状態"], [[item.id, item.code, item.name, FINANCE_ACCOUNT_TYPES.get(item.account_type, item.account_type), FINANCE_NORMAL_SIDES.get(item.normal_side, item.normal_side), item.system_key or "", "使用中" if item.active else "停止"] for item in chart_accounts])
    subaccounts_csv = finance_export_csv(["ID", "勘定科目ID", "勘定科目コード", "補助科目コード", "補助科目", "状態"], [[item.id, item.account_id, account_map[item.account_id].code if item.account_id in account_map else "", item.code, item.name, "使用中" if item.active else "停止"] for item in subaccounts])
    category_maps_csv = finance_export_csv(["区分", "費目コード", "費目", "勘定科目ID", "勘定科目コード", "勘定科目"], [["入金" if item.entry_type == "income" else "経費", item.category, FINANCE_CATEGORIES.get(item.category, item.category), item.account_id, account_map[item.account_id].code if item.account_id in account_map else "", account_map[item.account_id].name if item.account_id in account_map else ""] for item in category_maps])
    journals_csv = finance_export_csv(["伝票ID", "日付", "伝票番号", "摘要", "状態", "収支台帳ID", "取消元伝票ID", "登録者ID", "登録日時"], [[item.id, item.entry_date, item.voucher_no, item.description, item.status, item.source_entry_id or "", item.reversal_of_id or "", item.created_by_id, item.created_at.isoformat()] for item in journals])
    journal_lines_csv = finance_export_csv(["伝票ID", "日付", "伝票番号", "行番号", "借方貸方", "勘定科目ID", "勘定科目コード", "勘定科目", "補助科目ID", "補助科目コード", "補助科目", "金額", "摘要"], [[item.journal_entry_id, journal_map[item.journal_entry_id].entry_date, journal_map[item.journal_entry_id].voucher_no, item.line_no, FINANCE_NORMAL_SIDES.get(item.side, item.side), item.account_id, account_map[item.account_id].code if item.account_id in account_map else "", account_map[item.account_id].name if item.account_id in account_map else "", item.subaccount_id or "", subaccount_map[item.subaccount_id].code if item.subaccount_id in subaccount_map else "", subaccount_map[item.subaccount_id].name if item.subaccount_id in subaccount_map else "", item.amount, item.memo or ""] for item in journal_lines])
    ...
    with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as archive:
        for filename, content in [("chart-accounts.csv", chart_accounts_csv), ("subaccounts.csv", subaccounts_csv), ("category-account-maps.csv", category_maps_csv), ("journal-entries.csv", journals_csv), ("journal-lines.csv", journal_lines_csv), ("ledger.csv", ledger_csv), ("invoices.csv", invoice_csv), ("expense-requests.csv", expense_request_csv), ("entry-corrections.csv", correction_csv), ("cost-allocations.csv", allocation_csv), ("documents.csv", documents_csv), *document_files, *expense_document_files]:
            archive.writestr(filename, content); checksums[filename] = hashlib.sha256(content).hexdigest()
        manifest = {"schema_version": 2, "accounting_basis": "double_entry_accrual", "tenant_id": tenant.id, "tenant_name": tenant.name, "year": year, "period_start": year_start.isoformat(), "period_end": year_end.isoformat(), "exported_at": datetime.now(timezone.utc).isoformat(), "counts": {"chart_accounts": len(chart_accounts), "subaccounts": len(subaccounts), "category_maps": len(category_maps), "journal_entries": len(journals), "journal_lines": len(journal_lines), "ledger": len(entries), "invoices": len(invoices), "expense_requests": len(expense_requests), "expense_request_documents": len(expense_documents), "corrections": len(corrections), "allocations": len(allocations), "documents": len(documents)}, "checksums": checksums}
        archive.writestr("manifest.json", json.dumps(manifest, ensure_ascii=False, indent=2).encode("utf-8"))
    record_operation(session, "finance_export", "success", "会計・証憑一括出力", tenant.id, f"year={year} journals={len(journals)} lines={len(journal_lines)} ledger={len(entries)} invoices={len(invoices)} documents={len(documents)}")
    record_finance_audit(session, tenant.id, user.id, "finance_export", "finance_export", year, "複式仕訳・証憑一括出力を実行", f"journals={len(journals)} lines={len(journal_lines)} ledger={len(entries)} invoices={len(invoices)} documents={len(documents)}")
    session.commit()
